...
def check_finesse(folder):
    ...
    ax.plot(r[ix], sig[ix], 'b')
    ax.fill_between(r[ix], sig_mean-sig_std, sig_mean+sig_std, color='r', alpha=0.8)
    ax.fill_between(r[ix], sig_mean-2.0*sig_std, sig_mean+2.0*sig_std, color='r', alpha=0.4)
    ax.fill_between(r[ix], sig_mean-3.0*sig_std, sig_mean+3.0*sig_std, color='r', alpha=0.1)
...
if __name__ == "__main__":
    ...
    if rank == 0:
        ...
        parser.add_argument('--F_lim', '-F', type=float, nargs=2, default=[15.,25.], help='bounds for finesse fit. Default is 15-25')
        ...
        parser.add_argument('--Ti_Ar_lim', '-T', type=float, nargs=2, default=[0.025, 1.0], help='bounds for Ar Ti fit.  Default is 0.025-1.0 eV')
        ...
        if args.no_solve:
            ...
            solver_in = {'r':rr,'sig':ss,'basename':basename, 'error':error,
                    'F_lim':args.F_lim, 'livepoints':args.livepoints, 'Ti_lim': args.Ti_Ar_lim, 'Arel_lim': args.Arel_lim,
                    'Amp_lim':amp_lim, 'w0':[487.873302, 487.98634], 'mu':[232.03806, 39.948], 'Lpost':Lpost, 'dpost':dpost}
    else:
        solver_in = None

    solver_in = Comm.bcast(solver_in, root=0)
    if solver_in is not None:
        finesse_solver(solver_in['r'], solver_in['sig'], solver_in['error'], solver_in['Lpost'], solver_in['dpost'],
                solver_in['basename'], solver_in['F_lim'], solver_in['Amp_lim'], solver_in['Arel_lim'],
                solver_in['Ti_lim'], solver_in['w0'], solver_in['mu'], livepoints=solver_in['livepoints'])


    if rank == 0:
        check_finesse(args.folder)
