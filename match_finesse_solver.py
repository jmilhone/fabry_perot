...
def temp_match_solver(r, sig, sig_error, temp, temp_sigma, Lpost, dpost, basename, F_lim=(1.,18.), V_lim = [-2., 2.], livepoints=500, w0=487.98634, mu=39.948, errtemp=True):

    A_lim = [6., 10.]   #absolute amplitude of input spectrum (b4 convolution with Airy)
    O_lim = [-2., 5.]
    T_lim = [temp-10.*temp_sigma, temp+10.*temp_sigma]
    ixs = range(len(Lpost))
    ...
    if errtemp:
        ...
        def log_likelihood(cube, ndim, nparams):
            ...
            L = Lpost[j]
            d = dpost[j]
            pred = match_finesse_forward(r, L, d, cube[0], cube[4], cube[1], cube[2], errtemp=cube[5], w0=w0, mu=mu)
            pred += cube[3]
        ...
        def log_likelihood(cube, ndim, nparams):
            ...
            L = Lpost[j]
            d = dpost[j]
            pred = match_finesse_forward(r, L, d, cube[0], cube[4], cube[1], cube[2], errtemp=None, w0=w0, mu=mu)
            pred += cube[3]
            chisq = np.sum((sig - pred)**2 / sig_error**2) + ((cube[4] - temp)**2 / temp_sigma**2)
            return -chisq / 2.0
...
def match_finesse_check(folder, saveit=True, Ld_sub=20, F_sub=20, bins=30):
    ...
    if data['errtemp']:
        Fpost, Vpost, Apost, Opost, Tpost, Epost = read_match_finesse_results(folder, errtemp=True)
        nF = Fpost.size
        ...
        for i in range(Ld_sub):
            ...
            for j in range(F_sub):
                pred[:, i, j] = match_finesse_forward(rr, LL[i], dd[i], 
                          FF[j], TT[j], VV[j], AA[j],errtemp=EE[j],w0=data['wavelength'],mu=data['mu']) + OO[j]
    else:
        for i in range(Ld_sub):
            print '{0} of {1}'.format(i+1, Ld_sub)
            for j in range(F_sub):
                pred[:, i, j] = match_finesse_forward(rr, LL[i], dd[i], 
                          FF[j], TT[j], VV[j], AA[j],errtemp=None,w0=data['wavelength'],mu=data['mu']) + OO[j]
    ...
    if data['errtemp']:
        ...
        posts = [Fpost, Vpost, Apost, Opost, Tpost]
        names = ['F', 'V (km/s)', 'A (counts)', 'Offset (counts)', r'T$_{LIF}$ (eV)']
    ...
    if data['errtemp']:
        ...
        fig2,ax2 = plt.subplots(2,5,figsize=(16,6))
    ax2 = ax2.flatten()
...
if __name__ == "__main__":
    ...
    if rank == 0:
        ...
        parser.add_argument('--temp_sigma','-ts',type=float,default=0.015,help='known temperature standard deviation. Default is 0.015 eV')
