...
def solve_pcx_finesse_Ar(folder, data_fname, Ld_dir, order=0):
    def log_prior(cube, ndim, nparams):
        cube[0] = cube[0]*(F_lim[1]-F_lim[0]) + F_lim[0]
        cube[1] = cube[1]*(A_Th_lim[1]-A_Th_lim[0]) + A_Th_lim[0]

    def log_likelihood(cube, ndim, nparams):
        j = np.random.choice(i)
        L = Lpost[j]
        d = dpost[j]
        # pred = model.forward3(rr, L, d, cube[0], 0.025*1000.0/300.0, mu['Th'], w['Th'])
        # pred *= cube[1]

        pred = model.forward4(rr, L, d, cube[0], [0.025*1000.0/300.0, 0.2], 
                [mu['Th'], mu['Ar']], [cube[1], 3.0*cube[1]],
                [w['Th'], w['Ar']], V=0.0)
    ...
    A_Th_lim = [0.25*np.max(ss), 3.0*np.max(ss)]
    F_lim = [10.0, 25.0]
...
if __name__ == "__main__":
    ...
    solve_pcx_finesse_Ar(finesse_dir, calib_fname, Ld_dir, order=args.order)
