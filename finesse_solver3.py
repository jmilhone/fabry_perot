...
#extra_A = [0.068, 0.11, 0.044]
#extra_A = [0.068, 0.11, 0.038, 0.04, 0.04, 0.04]
extra_A = [0.06, 0.11, 0.03, 0.08, 0.01, 0.05]
extra_Ti = [0.025*1000.0/300.0 for x in range(len(extra_w))]
extra_V = [0.0 for x in range(len(extra_w))]
extra_mu = [232.03806 for x in range(len(extra_w))]
...
def check_finesse(folder, recover=False):
    ...
    if recover:
        try:
            ...
            sig_post = calculate_signal_post(new_r, Lpost, dpost, Fpost, Apost, Arelpost, Tipost, w0, mu)
            dict_2_h5(join(folder, "finesse_solver_model_post.h5"), {'signal post': sig_post, 'new_r': new_r})
    else:
        new_r = np.linspace(0, 900, 1000)
        #sig_post = calculate_signal_post(r[ix], Lpost, dpost, Fpost, Apost, Arelpost, Tipost, w0, mu)
        sig_post = calculate_signal_post(new_r, Lpost, dpost, Fpost, Apost, Arelpost, Tipost, w0, mu)
    ...
    for alpha, per in zip(alphas, keys):
        ...
        ax.fill_between(new_r, percentiles[per][0], percentiles[per][1], color='C3', alpha=alpha)
    ...
    fig1.tight_layout()
    fig1.savefig(join(folder, "plots", "finesse_1d_histograms.png"), dpi=400)
    plt.show(block=False)

    fig, ax = plt.subplots()
    #ax.plot(r**2, sig, 'C0', alpha=0.7)
    ax.plot(r, sig, 'C0', alpha=0.7)
    ...
    for alpha, per in zip(alphas, keys):
        ...
        #ax.fill_between(new_r**2, percentiles[per][0], percentiles[per][1], color='C3', alpha=alpha)
        ax.fill_between(new_r, percentiles[per][0], percentiles[per][1], color='C3', alpha=alpha)
    ...
    vals = forward_model(rr, Lmean, dmean, Fmean, w, mus,
            amp, Ti, V, sm_ang=False, nlambda=1024)
    #ax.plot(rr**2, vals, 'k')
    ...
    fig.tight_layout()
    fig.savefig(join(folder, 'plots/', 'finesse_fit.png'), dpi=400)
    ...
    for i in range(4):
        for j in range(i):
            print(i, j)
            fig, ax = plt.subplots()
            cb = my_hist2d(ax, post[:, i], post[:, j])
            ax.set_xlabel(axis_labels[j], fontsize=18)
            ax.set_ylabel(axis_labels[i], fontsize=18)
            cb.set_label("P({0},{1})".format(clabels[j], clabels[i]), fontsize=16)
            fig.tight_layout()
            fig.savefig(join(folder, "plots", "joint_{0}_{1}.png".format(clabels[j], clabels[i])), dpi=400)
            plt.show()
...
def _forward_model(r, Lpost, dpost, Fpost, Apost, Arelpost, Tipost, w0, mu, out=None, label=None):
    ...
    for i in range(n):
        ...
        sig[:, i] = forward_model(r, Lpost[i], dpost[i], Fpost[i], w, mus, amp,
                Ti, V, sm_ang=False, nlambda=1024) 
        #sig[:, i] += offsetpost[i]
    if out and label:
        out.put((label, sig))
    else:
...
if __name__ == "__main__":
    ...
    if solver_in is not None:
        finesse_solver(solver_in['r'], solver_in['sig'], solver_in['error'], solver_in['Lpost'], 
                ...
                #solver_in['mu'], livepoints=solver_in['livepoints'], resume=False)
