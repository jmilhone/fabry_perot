...
class PlotMarginalModes(object):
    ...
    def plot_conditional(self, dim1, dim2 = None, 
        ...
        cb = None 
        # determining min/max
        if all(numpy.isnan(mode['sigma'][dim1]) for mode in modes):
            min1 = 0.99*numpy.nanmin([mode['mean'][dim1] for mode in modes])
            max1 = 1.01*numpy.nanmax([mode['mean'][dim1] for mode in modes])
        else:
            min1 = numpy.nanmin([mode['mean'][dim1] - 3*mode['sigma'][dim1] for mode in modes])
            max1 = numpy.nanmax([mode['mean'][dim1] + 3*mode['sigma'][dim1] for mode in modes])
        if dim2 is not None:
            if all(numpy.isnan(mode['sigma'][dim2]) for mode in modes):
                min2 = 0.99*numpy.nanmin([mode['mean'][dim2] for mode in modes])
                max2 = 1.01*numpy.nanmax([mode['mean'][dim2] for mode in modes])
            else:
                min2 = numpy.nanmin([mode['mean'][dim2] - 3*mode['sigma'][dim2] for mode in modes])
                max2 = numpy.nanmax([mode['mean'][dim2] + 3*mode['sigma'][dim2] for mode in modes])
        ...
        if dim2 is not None:
            ...
            #im = ax.imshow(grid_z.transpose(), origin='lower', aspect='auto',
            #    cmap=cm.gray_r, alpha = 0.8, extent=(min1,max1,min2,max2))
            #cb = plt.colorbar(im)
            cf = ax.contourf(grid_x, grid_y, grid_z, alpha=0.8)
            cb = plt.colorbar(cf)
        else:
            #plt.xlim(min1, max1)
        ...
        if dim2 is not None:
            ax.contour(grid_x, grid_y, grid_z, levels[::-1], linewidths=0.5, colors='k')
        elif marginalization_type == 'max':
            # for the other types, these levels are pretty useless.
            for i in range(len(levels)):
                ax.plot([grid_x.min(), grid_x.max()], [levels[i]] * 2, '--', color='grey', label=leveltitles[i])
        
        # add points
        if with_points and dim2 is not None:
            ax.scatter(dim1_column, dim2_column, marker='+', color='black', s=1, alpha=0.4)
        # add ellipses
        i = 0
        for mode in modes:
            ...
            arrow = dict(facecolor='blue', width=0.1, headwidth=0.9,    
                linewidth=0.1)
            ax.annotate(text, xy=(ellipse_x, ellipse_y),
    ...
    def plot_modes_marginal(self, dim1, cumulative = False, grid_points = 200, 
        with_ellipses = True, with_points = True, ax=None):
        """
