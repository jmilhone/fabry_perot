...
def synthetic_data(plotit=0):
	...
	A=(1.+((4.*F**2.)/np.pi)*np.sin(2.*np.pi*((n*d)/lam)*(L/np.sqrt(L**2.+R**2.)))**2.)**-1.  #Ari function of rings
	...
	if plotit:
		...
		plt.imshow(A,vmin=0,vmax=1,cmap='gray',extent=[xmin,xmax,ymin,ymax],interpolation='nearest',origin='lower')
...
def real_data(plotit=0):
	...
	A=rgb[:,:,2].astype('float64')	#take only the blue pixel values, set as a float to remove worries about math later
...
def small_angle_sum(data,center,binsize=1):
	...
	R=np.sqrt(X**2.+Y**2.)	#R is the same shape and size as data, but lists the R location of pixels wrt center
	...
	rmax=np.min(np.abs([xmin,xmax,ymin,ymax]))			#the maximum r we want to go to is the nearest edge of CCD to center
	...
	binarr=np.fromfunction(lambda i: np.sqrt(2.*(i+1.)*rmax*binsize+(i+1.)*binsize**2.),(imax,),dtype='float64')
